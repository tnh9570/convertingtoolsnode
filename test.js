  ...
  try{
      ...
      glovalValue.map(async (data) => {
        ...
        INSERT INTO tcustomerpersonal (
          CUSTOMERID, CUSTNAME, CUSTJN, CUSTDOB, DOBTYPE, CUSTGENDER,
        ...
        const values = [data.CUST_NO, data.NAME, data.CTZN_NO,ctznno, 0, data.SEX, data.ENTR_DAY, data.MDFY_DAY, data.ADDR1 + data.ADDR2, currentDateTimeString]
...
async function main() {
    try {
        ...
        // customerData 함수 호출
        await customerData(glovalValue);
      } catch (error) {
        console.error(error);
      }
}
